 ...
 * This project incorporates and deletes contact details. The system 
...
public class ContactManagement {
	...
	void AddContacts() {
		...
		for (int i = 0; i < times; i++) {
			...
			// calls to DatabaseMgmt.java
			dbm.insertContact(cp);
		...
		for (int i = 0; i < cparry.size(); i++) {
			...
			if (cp.name.equals(name)) {
				...
				while (needModify == true) {
					...
					dbm.modifyContact(cp, modifyWhat);
